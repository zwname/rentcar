...
public class CarsController {
	...
	@ResponseBody
	public Message addCar(@RequestParam(value = "file", required = false) MultipartFile file,
			HttpServletRequest request, Cars cars) throws IOException {
	...
	public String allCars(HttpServletRequest request) {
		List<CarsCateBrandDto> findAll = carsService.findAll();
		request.getSession().setAttribute("allCars", findAll);
		return "front/allCars";
	}

	@RequestMapping("/goupdateCar")
	public String goupdateCar(@RequestParam("carsid") String carsid, HttpServletRequest request) {
		CarsCateBrandDto carsCateBrandDto = carsService.selectByPrimaryKey(carsid);
		request.getSession().setAttribute("carsCateBrandDto", carsCateBrandDto);
		return "back/cars/cars_update";
	}

	@RequestMapping(value = "/updateCars", method = RequestMethod.POST)
	@ResponseBody
	public Message updateCars(@RequestParam(value = "file", required = false) MultipartFile file,
			HttpServletRequest request, Cars cars) throws IOException {

		if (cars.getCarsid() != null) {
			if (file != null) {
				// 使用UUID给图片重命名，并去掉“-”
				String name = UUID.randomUUID().toString().substring(0, 8).replaceAll("-", "");
				// 获取文件的扩展名
				...
				cars.setImage("upfiles/" + name + "." + ext);
				if (carsService.updateByPrimaryKeySelective(cars) > 0) {
